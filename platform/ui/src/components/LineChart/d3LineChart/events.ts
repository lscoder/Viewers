...
import { zoomIdentity, zoomTransform, pointer, drag, bisector } from 'd3';
...
function mouseClosestPointIndex(gNodes, mousePoint) {
  const nodesLength = gNodes.length;
  const precision = 1;
  let bestIndex;
...
const _bindPointInteraction = (
  ...
  function _setDragListeners(
    ...
    function _startListener(event) {
      console.log('>>>>> _startListener :: event ::', event);
  ...
  const mouseDownListener = () => {
    const mousePoint = pointer(root.node());
